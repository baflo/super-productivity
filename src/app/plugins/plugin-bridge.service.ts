...
export class PluginBridgeService implements OnDestroy {
  ...
  async moveTaskToProject(taskId: string, targetProjectId: string): Promise<void> {
    typia.assert<string>(taskId);
    typia.assert<string>(targetProjectId);

    const project = await this._projectService
      .getByIdOnceCatchError$(targetProjectId)
      .toPromise();
    if (!project) {
    ...
    if (task.parentId) {
      throw new Error('Cannot move subtasks to another project');
    }

    this._taskService.moveToProject(task, targetProjectId);
    PluginLog.log('PluginBridge: Task moved to project', { taskId, targetProjectId });
  }
